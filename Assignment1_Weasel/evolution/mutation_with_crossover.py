...
def start_evolution_with_crossover():
    ...
    while True:
        ...
        if rand_survivor_fitness_1 > rand_survivor_fitness_2:
            parent1 = rand_survivor_1
        else:
            parent1 = rand_survivor_2

        rand_survivor_1, rand_survivor_fitness_1, _, rand_survivor_2, rand_survivor_fitness_2, _ \
            = evolve.get_random_survivors_from_population(survivor)

        if rand_survivor_fitness_1 > rand_survivor_fitness_2:
            parent2 = rand_survivor_1
        else:
            parent2 = rand_survivor_2
        ...
        for i in range(len(parent1)):
            ...
            if ''.join(child) not in evolve.generation_fitness:
                evolve.generation_fitness[''.join(child)] = evolve.check_fitness(child)

        mutated_child, number_of_mutations = evolve.mutate(child)
        str_child = ''.join(mutated_child)
...
if __name__ == '__main__':
    start_evolution_with_crossover()
